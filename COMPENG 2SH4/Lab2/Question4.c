...
void sortDatabySelection(struct Q4Struct array[], int size)
{
    ...
    for (int i = 0; i < size - 1; i++)
    {
        ...
        for (int j = i + 1; j < size; j++) // checking all the elements after i
        {
            if (array[j].intData < array[min].intData) // update minIndex if a smaller element is found
            {
                min = j;
            }
        }

        if (min != i) //if minIndex is no longer = i then swap the new minIndex with the first element in the unsorted area
        {
            temp = array[i];
            array[i] = array[min];
            array[min] = temp;
